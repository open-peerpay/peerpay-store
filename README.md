...
- `/` 默认商品首页，用户不需要登录；每个商品都有 `/products/:slug` 独立详情页，可直接分享和访问，商品描述按 Markdown 渲染，详情页内选择支付方式并填写可选备注后下单，提交后跳转 PeerPay 付款页。
- `/orders/:id` 是订单详情独立页，用户支付返回、历史订单查询或直接访问订单链接时，都在这里查看状态、卡密/发货内容和提货入口。
...
PeerPay 回调通过 HMAC-SHA256 验签后，Store 才会把订单从 `pending_payment` 更新为 `paid`，并执行卡密发货或动态上游取货。签名可放在 body 的 `sign` 字段或 `x-peerpay-signature` 请求头中。生产环境建议在后台配置 `Store 公开访问地址`，确保 PeerPay 可以访问回调 URL。
